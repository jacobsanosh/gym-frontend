...
async function assignWorkout(userEmail, workoutName, sets, reps) {
  const response = await client.post("/trainer/workout/assign", {
    userEmail,
    workoutName,
    sets,
    reps,
...
async function assignDiet(userEmail, dietName, noOfTimes, time) {
  const response = await client.post("/trainer/diet/assign", {
    userEmail,
    dietName,
    noOfTimes,
    time,
